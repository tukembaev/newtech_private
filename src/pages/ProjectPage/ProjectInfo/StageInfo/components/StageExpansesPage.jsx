...
import styles from 'pages/ProjectPage/components/ProjectExpansesPage.module.scss'
import { Layout } from 'components'
import { useSelector } from 'react-redux'
import right from 'assets/icons/chevron_right.png'
import { useNavigate } from 'react-router-dom'
import Notification from 'utils/Notifications'
import { getAllStageExpenses } from 'service/ProjectService'

const StageExpansesPage = () => {
  ...
  const projectInfo = useSelector((state) => state.project.stageInfo)
  const getData = async () => {
    try {
      let response = await getAllStageExpenses(projectInfo.id, data)
      ...
      <div className={styles.team__wrapper}>
        <div className={styles.team__header}>
          <div className={styles.title}>
            <span
              onClick={() => navigate(`/project/${projectInfo.project}`)}
            ...
            <span
              style={{ cursor: 'pointer' }}
              onClick={() => navigate(`/stage-info/${projectInfo.id}`)}
            >
              {' '}
              {projectInfo.title}
            </span>
          ...
          <table className={styles.table__wrapper}>
            <thead className={styles.table__head}>
              <tr className={styles.table__row}>
                <th className={styles.table__item}>
                  <span className={styles.table__title}>
                    ...
                    >
                      ...
                      <option value="увеличена прибыль">
                        Увеличение дохода
                      </option>
                    </select>
                  </span>
                ...
                <th className={styles.table__item}>
                  <span className={styles.table__title}>Исполнитель </span>
                </th>
                <th className={styles.table__item}>
...
export default StageExpansesPage
